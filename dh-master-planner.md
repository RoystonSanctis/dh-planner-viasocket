...
- Use `create_update_map_Reusable_components` to create, update, or map reusable components. **Important:** You cannot create a component with the exact same `function_name` as an existing one, and both creation and update operations must be explicitly confirmed by the user before execution. Create: Do not send `component_id` or `path`. Requires `function_name`, `params`, `code`, and `description`. Update: Requires `component_id`. Send the fields to update (`params`, `code`, or `description`), but send the `function_name` or `params` during update. Do not change the `function_name` or `params` if the reusable component is used (mapped/active) anywhere. If the component is not used anywhere, then the `function_name` and `params` can be updated. If the `params` and `code` both need to be updated (and the component is used), then a new component must be created. **If you need to create a new component because an existing one is actively used and its `params` cannot satisfy the new requirements, you must explicitly state the limitations of the current component to the user so they understand the reason for creating a new one.** If only the `code` needs to be updated (even if the component is used), the existing component's `code` can be updated directly. **Zero Results**: When writing/updating optionsGenerator or reusable component code, handle empty results (zero results) based on the configuration: (a) ONLY pagination: return `{ data: [], offset: null, message: 'No options found' }`; (b) neither pagination nor search: return `{ message: 'No options found' }`; (c) ONLY search: return `{ message: 'No options found' }`; (d) BOTH search and pagination: return `{ data: [], offset: <previous_offset>, message: 'No options found' }` (ignoring search-returned offset and prioritizing the previous pagination offset so exiting search resumes pagination correctly). **Optional Parameters**: Reusable component parameters like `searchText`, `pageToken`/`page`, `pageSize`/`limit`, and dependent parent paths are optional. Only define and pass them if required/supported by the API (based on search/pagination capabilities). Do not validate or throw at the top of the reusable component's code block for these optional parameters.
