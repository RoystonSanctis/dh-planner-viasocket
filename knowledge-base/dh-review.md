...
- **Generators & Reusable Components**: Return `{ message: <text> }` on zero results (never empty array) for generators (`optionsGenerator`/`fieldsGenerator`) and dropdown Reusable Components. Handle "parent not selected yet" (return warning block).
...
- **Zero Results for Generators & Reusable Components**: For `fieldsGenerator` / `optionsGenerator` and dropdown Reusable Components only: on zero results, return `{message: <user message>}`.
