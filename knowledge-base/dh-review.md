...
- **No AI Field in Triggers & Scheduled Filter Rule (CRITICAL)**: Triggers (Scheduled, Instant, Manual) strictly forbid `aifield` (`aifield` is permitted in Actions only). Flag any trigger containing an `aifield`. For scheduled triggers (`polling`), if the API supports filtering, verify that predefined filters are either exposed as input fields (Dropdown, Multiselect, Boolean, Input Group) for user configuration or added directly in the `perform` code (via query parameters or client-side filtering).
...
- **Allowed Types**: Dropdown, Input Group, Multi-select (all static/dynamic), Boolean, Text Input, HTML, Markdown, Dictionary, AI Field (Actions only — strictly forbidden in triggers), Number, Help, Help Static.
