...
- `canPaginate` and `enableSearchApi` are **conditional** — only set `canPaginate: true` if the list API supports pagination/offset parameters, and only set `enableSearchApi: true` if the list API supports a search query parameter. **Never include these flags when the API does not support those capabilities**, as doing so will cause errors at runtime. Omit them entirely (or set to `false`) when unsupported.
