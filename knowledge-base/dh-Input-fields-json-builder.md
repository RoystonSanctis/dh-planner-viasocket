...
- **Valid Use Cases**:
  ...
  4. **Manual Webhook Setup (`manual_webhook`)**: The single mandatory static `help` field containing two-part HTML webhook setup instructions (`🔗 Webhook Setup Guide` + `📤 What happens next?`). Strict format: Heading -> List. No intermediate text.
...
- **No Search, Only Pagination Empty Result & Success Rule (`canPaginate: true, enableSearchApi: false`):** When a dropdown only supports pagination without search capabilities, the output structure must always be `{ data: [...], offset: string|number|null }`. When the fetched array is empty (`items.length === 0`):
  - If initial page (`!currentOffset` or `offset` is null): return `{ data: [], offset: null, message: 'No <resources> found.' }`.
  - If subsequent page / end of pagination (`currentOffset && items.length === 0`): return `{ data: [], offset: null, message: '<Resources> Fetched Successfully' }`.
