...
# UX Field Ordering
Required fields first, optionals grouped after. Resource dropdowns use `canPaginate:true` **only if the list API supports pagination/offset parameters**, and `enableSearchApi:true` **only if the list API supports a search query parameter**. Do NOT add either flag unless the underlying API endpoint explicitly supports it — adding them when unsupported will cause runtime errors. **Help placement:** When using a static or dynamic help field, it must always be positioned below the field it is referring to.
...
| `boolean` | `options`, `customPlaceholder`, `customInputLabel`, `customHelp` | Exactly 2 options `{label,value}`, true-option FIRST. `defaultValue:{label,value}` must equal an option. Labels any binary pair; value mapping arbitrary. `customInputLabel` (very short, e.g., "Enter Pagination Option") and `customHelp` (detailed, e.g., "Enter 'true' to paginate results, or 'false' to fetch all users.") are required for manual mode, where `customHelp` specifies when to enter `true` and `false` values. (Optional: `placeholder`). |
| `dropdown`/`multiselect` static | `options`, `customInputLabel`, `customPlaceholder`, `customHelp` | `options:[{label,value,sample?,extraValue?}]` fixed. `defaultValue` = exact copy of an option (multiselect: array of copies). All three custom keys are required; `customInputLabel` must be very short (e.g. "Enter Spreadsheet ID" or "Enter Properties in Array") and `customHelp` must be detailed, guiding the user to enter the manual value/ID. The `help` key must describe selection. (Optional: `placeholder`). |
| `dropdown` dynamic | `optionsGenerator`, `customPlaceholder`, `customInputLabel`, `customHelp` | `canPaginate:true` (**only if** the list API supports pagination/offset) ⇒ generator must return `{data,offset,message?}`. `enableSearchApi:true` (**only if** the list API supports a search query parameter) ⇒ use `__searchText` in generator. Omit both flags entirely when the API does not support those capabilities — including them when unsupported causes errors. Generator returns `{message}` if no options found or to show warning block. Special Case: In the reusable component, if search is active (search query has value) and returns empty results, the `offset` returned must be the current cursor (`pageToken`/`offset` parameter) to preserve the previous offset when switching back to pagination (search API offset is ignored). All three custom keys are required; `customPlaceholder` = manual-input example (such as `123`); `customInputLabel` must be very short (e.g. `"Enter Spreadsheet ID"`); `customHelp` must be detailed (longer manual help/explain ID location, supports markdown links), guiding the user to enter the manual value/ID. The `help` key must describe selection. `defaultValue` = object `{label,value,sample}`. (Optional: `placeholder`). |
| `multiselect` dynamic | `optionsGenerator`, `customPlaceholder`, `customInputLabel`, `customHelp` | `customPlaceholder` = array example (such as `"[\"title\",\"status\"]"` or `["title","status"]`). `customInputLabel` = short manual label (e.g. `"Enter Properties in Array"` or `"Enter Column Name in Array"`). All three custom keys are required; `customInputLabel` must be very short and `customHelp` must be detailed (longer manual help/explain expected array format), guiding the user to enter the manual value/ID. The `help` key must describe selection. `defaultValue` = array of `{label,value,sample}`. (Optional: `placeholder`). |
