...
- **Response Return & Formatting (Base `.data` Extraction Rule)**: The actual raw response from any API called from viaSocket is ALWAYS accessible from the `response.data` key. The base return MUST always fetch from the `"data"` key—even if responses are collected or aggregated from multiple API calls, each API response must fetch from its own `"data"` key (e.g., `userRes.data`, `projectsRes.data`). Never access payload properties directly on the outer response object. From this base `"data"` key, you can then navigate to destination path keys as needed:
  ...
  - **Unnesting Nested Paths:** If the API response wraps actual data inside deep nested paths (e.g. `response.data.data`, `response.data.result.items`, `response.data.body.record`), extract and bubble up the actual payload from the base `response.data` to the root level instead of returning the cumbersome wrapper.
  - **Simplifying Huge / Bloated Responses:** When an API returns a massive payload filled with internal system attributes, raw binary buffers, or noisy metadata, simplify the output by returning selective essential keys from `response.data` (e.g. `id`, `name`, `status`, `created_at`, core values) to keep downstream pill-mapping clean.
  - **Flattening Complex Responses:** If deeply nested sub-objects create difficult mapping UX in viaSocket's variable pill picker, flatten key properties from `response.data` into a clean single-level or structured object.
  - **Decision Rule (What Best Matches):** Base on payload complexity: (1) Clean & direct → return `response.data`; (2) Nested actual data → unnest destination path from `response.data` to root; (3) Huge/bloated → pick selective keys from `response.data`; (4) Complex nested hierarchy → flatten destination keys for intuitive mapping; (5) Multi-item array → return array `[{...}]` for engine iteration. Always anchor on `.data` first.
...
- **Dropdown priority & UX preference (CRITICAL)**: In UX, always prefer dynamic dropdowns over plain text fields for resource references/IDs unless special cases apply: (1) category `DELETE` (strict exception — always direct text ID field of type `string`); (2) apps/workflows where IDs will specifically be passed from an upstream/previous step or where no options-fetching API exists. Never use a string field if an options-fetching API exists. No exceptions for UPDATE.
...
- **Dropdown rule**: Always prefer dropdowns when options can be fetched (using `canPaginate`/`enableSearchApi` when supported by API). Use direct string ID field only for DELETE or when IDs are passed from upstream steps with no options-fetching API. **Endpoint Parameter Verification**: Verify the GET API endpoint in the dropdown build for `sort`, `limit`, and `search` query parameter availability against official API documentation; never assume parameter availability without documented reference.
...
- **Default Value Rule**: Avoid using the `defaultValue` key in the input JSON fields. Instead, clearly mention the default value in the field's `help` text, and apply the default value fallback inside the perform code (e.g. `const limit = context.inputData.limit || 10;`).
...
- **Optional Fields Chooser & Input Group Organization (MANDATORY UX PRACTICE)**: When there are multiple optional fields in the UI, follow the UX practice of having a single multiselect field chooser (e.g. "Select Additional Fields"). Always prioritize organizing optional fields into **Input Groups** in the UX and attach the `visibilityCondition` to the input group. If grouping into input groups is not possible, list the individual fields in the multiselect, and each field must have the `visibilityCondition` attached. It is always recommended to organize fields in input groups in the UX.
...
# UX Field Ordering
Required first, optionals grouped after. **Optional Fields & Multiselect Field Chooser Rule (Mandatory UX Practice)**: If there are multiple optional fields in the UI, use a single multiselect and group all the fields in an Input Group with the `visibilityCondition` on the group. If grouping is not possible, individual fields should be listed in the multiselect, and each field must have the `visibilityCondition`. It is always recommended to organize fields in input groups in the UX to prevent UI bloat and maintain a clean hierarchy. **`canPaginate`/`enableSearchApi` priority**: (1) both → both true; (2) search only → paginate false, search true; (3) pagination only → paginate true, search false; (4) neither → both false. Set flags true if reusing a component that implements them. **Strict API Parameter Verification**: Verify the GET API endpoint in the dropdown build for `sort`, `limit`, and `search` query parameter availability against documented API reference—never assume availability without documented confirmation. **Help placement**: static/dynamic help field always below the field it refers to.
...
| Manual | HelpStatic (only allowed field — two-part HTML instructions: 🔗 Webhook Setup Guide + 📤 What happens next? Strict format: Heading -> List. No intermediate text.) |
...
- **CREATE/UPDATE**: Chooser Multiselect feeds fields group. Static/known fields → static input groups with `visibilityCondition` per field/group on Multiselect. `fieldsGenerator` only for truly dynamic fields; return `{message:'Select a resource first.'}` if deps missing. Map API types correctly. State standard default fields in `help` and handle fallbacks in perform code (avoid `defaultValue` key in JSON); force-include mandatory fields. Line items → repeating Input Groups. Sections → Multiselect entries + conditional Input Groups. Reference vs inline → Boolean/Dropdown fork. UPDATE help: "unfilled fields stay unchanged".
...
| `dropdown` dynamic | `optionsGenerator`, `customPlaceholder`, `customInputLabel`, `customHelp` | Configure `canPaginate`/`enableSearchApi` per API capability. **Strict Output Structures**: (1) If `canPaginate: true`, `enableSearchApi: true`, or both $\rightarrow$ output MUST ALWAYS be `{ data: [{label, value, sample}], offset: string|number|null }` (`offset` is `null` when no further pages exist). (2) If both `canPaginate: false` and `enableSearchApi: false` (or omitted, default `false`) $\rightarrow$ output MUST ALWAYS be `[{ label, value, sample }]`. Search $\rightarrow$ use `__searchText`. Empty/info handling: (a) No pagination/search $\rightarrow$ `{message}`; (b) **Pagination only (`canPaginate: true, enableSearchApi: false`)** $\rightarrow$ strictly `{data: [], offset: null, message}`: on initial fetch (`!currentOffset && length === 0`) return `{data: [], offset: null, message: 'No <resources> found.'}`, on pagination end (`currentOffset && length === 0`) return `{data: [], offset: null, message: '<Resources> Fetched Successfully'}`; (c) Both pagination and search $\rightarrow$ `{data:[],offset:previous_offset,message}` (preserve pagination offset on empty search). `customPlaceholder` = sample value (e.g. `123`). `customInputLabel` must not start with "Enter" (e.g. "Spreadsheet ID"). `customHelp` = "Enter the ID/value… from actions like List, Find…". `defaultValue` = `{label,value,sample}`. |
| `multiselect` dynamic | `optionsGenerator`, `customPlaceholder`, `customInputLabel`, `customHelp` | **No `canPaginate`/`enableSearchApi` support.** Output structure is STRICTLY a fixed array `[{ label, value, sample }]`. If calling a paginated reusable component (`{data, offset}`), `optionsGenerator` MUST perform client-side pagination (looping internally to fetch and aggregate all pages/results) and return the aggregated array. `customPlaceholder` = array example (e.g. `"[\"title\",\"status\"]"`). `customInputLabel` must not start with "Enter". `defaultValue` = array of `{label,value,sample}`. |
...
| `help` static | — (`key`·`type`·`help` only) | Content: text/HTML/MD. No `label`/`required`/`placeholder`. Optional: `visibilityCondition`. Position below referring field. **Add ONLY when strictly necessary** for critical notices (DELETE irreversible warnings, behavior-changing selections, mandatory external prerequisites, manual webhook HTML setup, or complex polling math). |
| `help` dynamic | `source` | `source` JS → `{message}`. Not `optionsGenerator`. Optional: `label`, `visibilityCondition`. No `required`/`placeholder`/`help`. Position below referring field. **Add ONLY when strictly necessary** for dynamic validation/permission checks or contextual warnings. |
...
**Options metadata**: `sample` MUST equal `value`; include only when value is ID ≠ label. Dynamic `defaultValue` requires `sample` (note: Avoid using the `defaultValue` key in input JSON; explain defaults in `help` and apply fallbacks in perform code). `extraValue` = hidden metadata (any data type), read via `context?.inputData?.{key}_extraValue` (group: `{group}?.{key}_extraValue`).
...
# Minimalism
- **Input Field `help` property**: Always required on standard input fields — concise, non-technical. Starts with "Enter" for string/number/dictionary/date/aifield/markdown/html/input groups (for `string` fields, especially ID string fields like `parent_task_id`, `help` MUST say `"Enter [ID]..."`, NOT `"Select from the list"` or `"Select..."`); "Select" for dropdown/multiselect/boolean. `customHelp`/`customInputLabel`/`customPlaceholder` are mandatory for boolean/dropdown/multiselect.
- **Standalone `help` field (`type: "help"`) — Strict Necessity Rule (CRITICAL)**: A standalone `help` field (`type: "help"`, whether static or dynamic) is a specialized UI banner / notice block and MUST ONLY be added when strictly needed for very important notices or mandatory structural requirements. ❌ NEVER add standalone `help` fields for ordinary explanations or standard field descriptions (use the field's own `help` property instead).
  - **Only use a standalone `type: "help"` field in these specific cases**:
    ...
    4. **Manual Webhook Setup (`manual_webhook`)**: The single mandatory static `help` field containing two-part HTML webhook instructions (`🔗 Webhook Setup Guide` + `📤 What happens next?`). Strict format: Heading -> List. No intermediate text.
...
### 🔑 The Base `.data` Extraction Rule (CRITICAL)
- **Always anchor on `.data`**: In viaSocket, the raw response returned by `axios` is an Axios response object. The actual API payload is ALWAYS located inside the `"data"` key (`response.data` or `{ data } = response`).
- **Never access payload keys directly on the response wrapper**: Writing `response.id`, `response.records`, or `response.status_code` to read API payload fields is WRONG (`response` contains HTTP metadata like `status`, `headers`, `config`).
- **Multiple API Calls**: When collecting or aggregating responses from multiple API calls (e.g. fetching a user, then their projects, or combining paginated chunks), **every single API call MUST fetch its payload from its own `"data"` key**:
  ```javascript
  const userRes = await axios.get('<url>/user');
  const projectsRes = await axios.get('<url>/projects');

  // Both base responses MUST be extracted from the .data key:
  const userData = userRes.data;
  const projectsData = projectsRes.data;

  // Navigate to destination paths from each:
  return {
    success: true,
    user_id: userData?.id,
    user_name: userData?.profile?.full_name,
    projects: projectsData?.items || projectsData || []
  };
  ```
- **Navigate to Destination Path Keys**: Once the base response is extracted from the `"data"` key, you can continue navigating deeper into any destination path keys (e.g., `response.data.data.items`, `response.data.result.records`) to unnest, flatten, or select specific keys.
...
- **Output Formats**: Standard (non-paginated) returns `[{ label, value, sample }]`. Paginated returns `{ data: [{ label, value, sample }], offset: string|number|null }`.
- **API Capability & Parameter Verification (Strict Documentation Rule)**: Verify the GET API endpoint used in the dropdown build specifically for `sort`, `limit`, and `search` query parameter availability against official API documentation. Never assume availability without documented API reference. If the API supports pagination, search, or both, the dynamic dropdown field configuration MUST set `canPaginate` and/or `enableSearchApi` to `true`, and return `{ data, offset }`.
- **Client-Side Pagination Handling**: If a reusable component is designed for pagination (returning `{ data, offset }`) and is consumed in a non-paginated dynamic dropdown (`canPaginate: false`) or a dynamic multiselect, the calling `optionsGenerator` MUST implement client-side pagination (looping internally to fetch and aggregate all pages) and return the aggregated flat array `[{ label, value, sample }]`.
- **Parameters (Strict Documentation Rule)**: Only add `offset` (or `pageToken`), `limit` (or `pageSize`), or `search` (`searchText`) parameters if the API explicitly supports them and they are documented in the API reference. Never add assumed or unverified parameters. Don't validate optional params at top. Map dynamic values to params; never read `context.inputData`/`__searchText`/`context.paginateData` directly inside component.
- **Zero Results**: Return `{ message: "No [resources] found." }` (or `{ data: [], offset: null, message: "…" }` if paginated).
...
- **P1 (Automation)**: No raw IDs typed by user (use dropdowns) · verify dropdown GET API endpoint for `sort`, `limit`, and `search` availability against documented API reference (no assumptions) · reusable components only include `offset`, `limit`, or `search` params if documented & supported · handle pagination on lists · flag internal pagination unless "list all" or user-enabled · flag non-idempotent/repeat-unsafe actions · avoid `Promise.all` for rate-limited calls (sequential + delay; API rate limit counter logic acceptable).
- **P2 (UX)**: Required fields first · optional fields managed via single multiselect chooser (organize into Input Groups with `visibilityCondition`; fallback to individual fields with `visibilityCondition` if grouping not possible; always recommend input groups) · mention default values in `help` & apply fallbacks in perform code (avoid `defaultValue` key in input JSON) · hide complexity (auto-detect format).
...
- Perform: wrapper correct · `axios`/`fetch` only · `context.inputData.<key>` mapped · endpoint matches docs · required-field guards · rate-limit handled · base return always fetches from the `.data` key of each API response (including across multiple API calls), then navigates to destination paths (raw `response.data`, metadata injection with `success`/`id`/`has_more`, unnested actual data, selective keys for bloated responses, or flattened complex objects).
...
- Default values: Avoid using the `defaultValue` key in input JSON fields. Clearly state default values in the field's `help` text, and apply default value fallbacks inside the perform code.
- Optional fields UX practice: If multiple optional fields exist in the UI, verify a single multiselect field chooser is used to control visibility. Verify fields are organized into Input Groups with `visibilityCondition` applied (or individual fields with `visibilityCondition` if grouping is not possible). Confirm that organizing fields in input groups is prioritized in the UX.