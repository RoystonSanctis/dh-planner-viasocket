...
| `dropdown` dynamic | `optionsGenerator`, `customPlaceholder`, `customInputLabel` | `canPaginate:true` (paginated list) ⇒ generator returns `{data,offset}`. `enableSearchApi:true` ⇒ use `__searchText`. `customPlaceholder` (required) = manual-input example (e.g. `E.g., 123`). `customInputLabel` (required) = short manual label (e.g. `Enter Spreadsheet ID`). Optional: `customHelp` (longer manual help/explain ID location, supports markdown links). `defaultValue` = object `{label,value,sample}`. |
