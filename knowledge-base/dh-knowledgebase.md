...
- **Output / `inputjson` Format**: `inputjson` is an **Object** structured as `{"steps": {}, "blocks": {}, "inputFields": [...]}`. The `inputFields` value is strictly an **Array of Objects** (`[...]`), where each element in the array is a field configuration object. The `steps` and `blocks` values are **Objects** (always empty `{}`). Never pass stringified objects for `steps`/`blocks`. Ignore auto-generated `steps`/`blocks`/`dependsOn`.
