...
- **Dropdown rule**: Always prefer dropdowns when options can be fetched (using `canPaginate`/`enableSearchApi` when supported by API). Use direct string ID field only for DELETE or when IDs are passed from upstream steps with no options-fetching API.
...
- **API Capability & Dropdown Flags**: If the API supports pagination, search, or both, the dynamic dropdown field configuration MUST set `canPaginate` and/or `enableSearchApi` to `true`, and return `{ data, offset }`.
