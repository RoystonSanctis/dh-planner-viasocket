...
- **Output / `inputjson` Format**: When building `inputjson` for API tool calls, wrap as `{"steps": {}, "blocks": {}, "inputFields": [...]}`. `inputFields` is the array of raw field JSON objects. `steps` and `blocks` MUST always be empty objects `{}` as these keys are auto-generated by the backend. Never wrap `inputFields` directly or pass stringified objects for `steps`/`blocks`. Ignore auto-generated `steps`/`blocks`/`dependsOn`.
