...
| `dropdown`/`multiselect` static | `options`, `customInputLabel`, `customPlaceholder` | `options:[{label,value,sample?,extraValue?}]` fixed. `customInputLabel` and `customPlaceholder` are required for manual input mode. `defaultValue` = exact copy of an option (multiselect: array of copies). |
| `dropdown` dynamic | `optionsGenerator`, `customPlaceholder`, `customInputLabel` | `canPaginate:true` (paginated list) ⇒ generator returns `{data,offset,message?}`. `enableSearchApi:true` ⇒ use `__searchText`. Generator returns `{message}` if no options found or to show warning block. `customPlaceholder` (required) = manual-input example (e.g. `E.g., 123`). `customInputLabel` (required) = short manual label (e.g. `Enter Spreadsheet ID`). Optional: `customHelp` (longer manual help/explain ID location, supports markdown links). `defaultValue` = object `{label,value,sample}`. |
| `multiselect` dynamic | `optionsGenerator`, `customPlaceholder`, `customInputLabel` | `customPlaceholder` (required) = array example (e.g. `E.g., ["title","status"]`). `customInputLabel` (required) = short manual label (e.g. `Enter Column Name in Array`). Optional: `customHelp` (longer manual help/explain expected array format). `defaultValue` = array of `{label,value,sample}`. |
