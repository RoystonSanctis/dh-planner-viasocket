...
| `dropdown` dynamic | `optionsGenerator`, `customPlaceholder`, `customInputLabel`, `customHelp` | `canPaginate:true` (paginated list) ⇒ generator returns `{data,offset,message?}`. `enableSearchApi:true` ⇒ use `__searchText`. Generator returns `{message}` if no options found or to show warning block. Special Case: In the reusable component, if search is active (search query has value) and returns empty results, the `offset` returned must be the current cursor (`pageToken`/`offset` parameter) to preserve the previous offset when switching back to pagination (search API offset is ignored). All three custom keys are required; `customInputLabel` must be very short (e.g. "Enter Spreadsheet ID") and `customHelp` must be detailed, guiding the user to enter the manual value/ID. The `help` key must describe selection. `defaultValue` = object `{label,value,sample}`. (Optional: `placeholder`). |
