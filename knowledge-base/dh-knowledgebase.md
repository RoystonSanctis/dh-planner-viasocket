...
- **Output / `inputjson` Format**: `inputjson` in `request_payload` is an **Object** structured as `{"steps": {}, "blocks": {}, "inputFields": [...]}`. `steps` and `blocks` MUST strictly be raw empty objects (`{}`) — NEVER stringified (❌ NO `"{}"` or `"{\}"`). `inputFields` MUST strictly be an **Array** (`[...]`) — NEVER an object (❌ NO `{}`). Empty fields = empty array `[]` (❌ NO `"inputFields": {}`). Ignore auto-generated `steps`/`blocks`/`dependsOn`.
