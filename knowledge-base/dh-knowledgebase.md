...
| `dropdown` dynamic | `optionsGenerator`, `customPlaceholder`, `customInputLabel`, `customHelp` | `canPaginate:true` (**only if** the list API supports pagination/offset) ⇒ generator must return `{data,offset,message?}`. `enableSearchApi:true` (**only if** the list API supports a search query parameter) ⇒ use `__searchText` in generator. Omit both flags entirely when the API does not support those capabilities — including them when unsupported causes errors. Special Case: In the reusable component, if search is active (search query has value) and returns empty results, the `offset` returned must be the current cursor (`pageToken`/`offset` parameter) to preserve the previous offset when switching back to pagination (search API offset is ignored). All three custom keys are required; `customInputLabel` must be very short (e.g. "Enter Spreadsheet ID") and `customHelp` must be detailed, guiding the user to enter the manual value/ID. The `help` key must describe selection. `defaultValue` = object `{label,value,sample}`. (Optional: `placeholder`). |
| `multiselect` dynamic | `optionsGenerator`, `customPlaceholder`, `customInputLabel`, `customHelp` | `customPlaceholder` = array example (such as `"[\"title\",\"status\"]"`). `customInputLabel` = short manual label (e.g. "Enter Properties in Array"). All three custom keys are required; `customInputLabel` must be very short and `customHelp` must be detailed, guiding the user to enter the manual value/ID. The `help` key must describe selection. `defaultValue` = array of `{label,value,sample}`. (Optional: `placeholder`). |
