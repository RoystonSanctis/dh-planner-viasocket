...
| `dropdown` dynamic | `optionsGenerator`, `customPlaceholder` | `canPaginate:true` (paginated list) ⇒ generator returns `{data,offset}`. `enableSearchApi:true` ⇒ use `__searchText`. `customPlaceholder` (compulsory) = manual-input example (e.g. `E.g., 123`). Optional: `customInputLabel` (short manual label, e.g. `Enter Spreadsheet ID`), `customHelp` (longer manual help/explain ID location, supports markdown links). `defaultValue` = object `{label,value,sample}`. |
| `multiselect` dynamic | `optionsGenerator`, `customPlaceholder` | `customPlaceholder` (compulsory) = array example (e.g. `E.g., ["title","status"]`). Optional: `customInputLabel` (short manual label, e.g. `Enter Column Name in Array`), `customHelp` (longer manual help/explain expected array format). `defaultValue` = array of `{label,value,sample}`. |
