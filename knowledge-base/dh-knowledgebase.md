...
| `dropdown` dynamic | `optionsGenerator`, `placeholder`, `customPlaceholder`, `customInputLabel` | `canPaginate:true` (paginated list) ⇒ generator returns `{data,offset,message?}`. `enableSearchApi:true` ⇒ use `__searchText`. Generator returns `{message}` if no options found or to show warning block. `customPlaceholder` = manual-input example (such as `123`). `customInputLabel` = short manual label (such as `Enter Spreadsheet ID`). Optional: `customHelp` (longer manual help/explain ID location, supports markdown links). `defaultValue` = object `{label,value,sample}`. |
