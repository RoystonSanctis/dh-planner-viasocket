...
| `dropdown` dynamic | `optionsGenerator`, `placeholder`, `customPlaceholder`, `customInputLabel` | `canPaginate:true` (**only if** the list API supports pagination/offset) ⇒ generator must return `{data,offset,message?}`. `enableSearchApi:true` (**only if** the list API supports a search query parameter) ⇒ use `__searchText` in generator. Omit both flags entirely when the API does not support those capabilities — including them when unsupported causes errors. Generator returns `{message}` if no options found or to show warning block. `customPlaceholder` = manual-input example (such as `123`). `customInputLabel` = short manual label (such as `Enter Spreadsheet ID`). Optional: `customHelp` (longer manual help/explain ID location, supports markdown links). `defaultValue` = object `{label,value,sample}`. |
| `multiselect` dynamic | `optionsGenerator`, `placeholder`, `customPlaceholder`, `customInputLabel` | `customPlaceholder` = array example (such as `["title","status"]`). `customInputLabel` = short manual label (such as `Enter Column Name in Array`). Optional: `customHelp` (longer manual help/explain expected array format). `defaultValue` = array of `{label,value,sample}`. |
