...
### Instant Trigger Common Input Fields
- **Dropdown Dynamic** — Used for selecting the resource to listen to (e.g., Page, Form, Channel). Configure `canPaginate` and `enableSearchApi` flags using the priority rules based on API capability: (1) both search and pagination supported ⇒ `canPaginate:true, enableSearchApi:true`; (2) search only, no pagination ⇒ `canPaginate:false, enableSearchApi:true`; (3) pagination only, no search ⇒ `canPaginate:true, enableSearchApi:false`; (4) neither ⇒ `canPaginate:false, enableSearchApi:false`. Verify the GET API endpoint specifically for `sort`, `limit`, and `search` query parameter availability against documented API reference (never assume query parameter support); if reusing an existing component that implements pagination/search, set the corresponding flags to `true`.
...
*   **Mandatory Two-Part HTML Structure in `help`:** The `help` HTML content MUST strictly follow a 2-part structure with standard inline styling (`<div style="font-family: Arial, sans-serif; line-height: 1.6;">`):
    1. **`🔗 Webhook Setup Guide`**: Followed strictly by a list (`<ul>` / `<ol>` and `<li>`) guiding the user where in the external platform dashboard to navigate, create/connect the webhook, select events, paste the copied **Webhook URL**, and save. (Strict format: Heading -> List. No intermediate text/paragraphs).
    2. **`📤 What happens next?`**: Followed strictly by a list (`<ul>` and `<li>`) explaining what event data the external service will automatically dispatch to this webhook URL once connected. (Strict format: Heading -> List. No intermediate text/paragraphs).
...
* **General Principles:**
  ...
  * **Field Chooser & Optional Fields Pattern (MANDATORY UX PRACTICE):** When there are multiple optional fields in the UI:
    - Use a single **Multiselect** ("Select Additional Fields" / "Fields to Include") to control their display.
    - **Organize fields in Input Groups (Always Recommended):** It is always recommended to organize optional fields into **Input Groups** in the UX. Group related fields together inside an Input Group and add the `visibilityCondition` to the entire input group based on whether its option is selected in the multiselect (e.g. `Array.isArray(context?.inputData?.additional_fields) && context.inputData.additional_fields.includes('address_details')`).
    - **Individual Fields Fallback:** If logical grouping into input groups is not possible, list the individual optional fields as separate options in the multiselect, and attach a `visibilityCondition` directly to each individual field.
    - *Always prioritize organizing fields into input groups in the UX to prevent visual clutter, maintain a clean form hierarchy, and avoid overwhelming users.*
...
* **Dropdown Design Rules:**
  ...
  * **No UPDATE Bypass Exceptions:** There are no exceptions for UPDATE actions; they must also use dropdowns/multiselects if options can be fetched, regardless of whether parent dropdowns are required.
  * **GET API Endpoint & Reusable Component Parameter Verification (CRITICAL & STRICT):**
    * **Verify GET Endpoint Parameter Availability:** When building dynamic dropdowns, verify the GET API endpoint used to fetch options specifically for the availability of `sort`, `limit`, and `search` query parameters against documented API references. **Do not assume parameter availability**; a documented API reference is strictly required before configuring search, limit, or sort capabilities.
