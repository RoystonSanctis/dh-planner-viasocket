...
- **Key naming (Static vs. Dynamic)** — For static input fields, the `key` must never contain a dot (`.`). For dynamic fields generated within dynamic input groups via `fieldsGenerator`, they CAN contain a dot (`.`), and dot-to-underscore normalization is not required.
