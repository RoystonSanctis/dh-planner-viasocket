...
- **Authentication Handling:** All authentication is passed from the backend via connections (`authenticationpaths`). Do NOT flag if auth is not passed in the API payload. However, you MUST flag if the auth path is directly used in the code. Only use `context?.authData` for non-auth keys like domains or IDs.
