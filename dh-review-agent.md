...
- **Authentication Handling:** All authentication is passed from the backend via connections (`authenticationpaths`). Strictly do not include the authentication path in the code, as it is passed from the backend. Do NOT flag if auth is not passed in the API payload. However, you MUST flag if the auth path is directly used in the code. Only use `context?.authData` for non-auth keys like domains or IDs.
...
- **Reusable Component Validation Checks:** In reusable components called by dynamic dropdowns, validation checks for required parameters/parent dependencies must ALWAYS `throw` a structured fallback object (e.g. `if (!workspaceId) { throw { data: [], offset: null, message: 'Select a workspace first.' }; }`), never a generic Error. The calling `optionsGenerator` must wrap the call in `try { return await fetchComponent(...); } catch (error) { await errorComponent(error); }`.
...
- **Response Return Structure (Base `.data` Extraction Rule):** The actual raw API response is always accessible from the `response.data` key. The base return must always extract from the `"data"` key (including when aggregating responses from multiple API calls, e.g. `userRes.data`, `projectsRes.data`), and then navigate to destination path keys as needed. Perform code can inject metadata (`success`, `id`, `has_more`), unnest actual data, filter bloated responses with selective keys, or flatten complex objects to make the output response more structured or organised for the user/flow. Do not flag this as an issue.
- **No AI Field in Triggers & Scheduled Filtering (CRITICAL):** Triggers (Scheduled, Instant, Manual) must NEVER contain an AI field (`aifield`). `aifield` is strictly for Actions. If reviewing a scheduled trigger and the target API supports filtering, ensure that predefined filters are either fetched from input fields (Dropdown, Multiselect, Boolean, Input Group) or applied directly in the `perform` code (via query parameters or client-side filtering).
